(function($) {
    $.picasa = {
        albums: function(user, callback) {
            var url = "http://picasaweb.google.com/data/feed/base/user/:user_id?alt=json&kind=album&hl=en_US&access=visible&fields=entry(id,published,media:group(media:thumbnail,media:description,media:keywords,media:title))&callback=?";
        ...
        images: function(user, album, callback) {
            var url = "http://picasaweb.google.com/data/feed/base/user/:user_id/albumid/:album_id?alt=json&kind=photo&hl=en_US&fields=title,entry(title,gphoto:numphotos,media:group(media:description,media:content,media:thumbnail))&callback=?";
            url = url.replace(/:user_id/, user).replace(/:album_id/, album);
            var image = null;
            var images = [];
            $.getJSON(url, function(data) {
                var gallery = data.feed.title["$t"];
                $.each(data.feed.entry, function(i, element) {
